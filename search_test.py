...
class PubMedSearcher:
    ...
    def __init__(self, email, api_key=None):
        ...
        Args:
            ...
            api_key (str, optional): use one's own api key
        """
    ...
    def search_pubmed(self, term, retmax=100, sort="relevance"):
        """
        执行PubMed检索
...
def main():
    try:
        ...
        search_terms = {
            "combined_query": (
                "(traditional Chinese medicine OR TCM OR TCM-components OR TCM-formula OR natural factors) AND "
                "(nanomedicine OR nanoparticles OR nanotechnology OR nanocarrier OR delivery carriers) AND "
                "(pulmonary delivery OR aerosol OR inhalation OR pulmonary)"
            )
        }
        ...
        searcher = PubMedSearcher(
            email="use one's own email address",
            api_key="use one's own api key"
        )
